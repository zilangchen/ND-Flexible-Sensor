...
/**
  ...
  * @version V1.0
  * @date    2022-3-10
  * @brief   IIC OLED应用函数接口
  ******************************************************************************
  * @attention
  *
  * 实验平台:零涂LT_STM32_Kit开发板搭配零涂FDC2214模块
  * 管脚    :VCC:3.3V GND SDA:B9 CLK:  B8  （修改管教见lt_fdc2214_iic.h下的修改管脚）
  * 使用示例:
             double cap[4] = {0};
             int i = 0;
             ...
             while(1)
             {
                 for(i = 0; i < 4; i++)
                 {
                     cap[i]=getcap(i,18,33);//采集数据
                     printf("cap[%d]=%lfpf,", i, cap[i]);
                 }
                 printf("\r\n");
                 delay_ms(500);
             }
...
double FDC2214_CalculateCapacitance(double frequency, float inductance, float capacitance)
{
    ...
    cap = (1000000000000000000/ (inductance * (2 * PI * frequency) * (2 * PI * frequency)))-capacitance;
    return cap;
}
...
uint8_t FDC2214_Init(void)
{
    ...
    if(deviceID == FDC2214_ID)
    {
        ...
        FDC2214_SetRcount(FDC2214_Channel_0, 10000);//4000us 转换周期
        FDC2214_SetRcount(FDC2214_Channel_1, 10000);//4000us 转换周期
        FDC2214_SetRcount(FDC2214_Channel_2, 10000);//4000us 转换周期
        FDC2214_SetRcount(FDC2214_Channel_3, 10000);//4000us 转换周期
        ...
        FDC2214_SetClockSource(1);//内部时钟源
        FDC2214_SetHighCurrentMode(0);//正常电流传感器驱动

        FDC2214_SetMUX_CONFIG(1, FDC2214_Channel_Sequence_0_1_2_3, FDC2214_Bandwidth_10M);//开启0123通道采集，打开自动扫描，10MHz带宽
...
double getcap(u8 channel,double dg,double dr)
{
    double Cap=0.0;
    switch(channel)
    {
